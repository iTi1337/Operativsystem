...
struct dir_entry {
    char file_name[56]; // name of the file / sub-directory 56
    uint32_t size; // size of the file in bytes 4
    uint16_t first_blk; // index in the FAT for the first block of the file 2
    uint8_t type; // directory (1) or file (0) 1
    uint8_t access_rights; // read (0x04), write (0x02), execute (0x01) 1 
}; //total size = 56 + 4 + 2 + 1 + 1 = 64 bytes the maximum 4096 / 64 =ish 64 (rounded down) total entires
